...
function loadStoredConstellations(): ConstellationConfig | null {
  if (typeof window === "undefined") return null;
  const data = loadPipelineConstellations() ?? safeParseJson<ConstellationConfig>(localStorage.getItem(CONSTELLATION_STORAGE_KEY));
...
export default function CuratePage() {
  ...
  const [status, setStatus] = useState("Load the Assign arrangement draft or import arrangement and constellations JSON.");
  ...
  useEffect(() => {
    ...
    if (storedArrangement) {
      ...
      setStatus(storedConstellations ? "Restored Curate inputs from the Builder pipeline." : "Restored arrangement draft. Artwork edits will create constellations.json.");
    }
  }, []);
  ...
  const selectedCenter = useMemo(() => centerToMapPoint(selectedItem, selectedBook ? centroidForBook(selectedBook) : null), [selectedBook, selectedItem]);
  ...
  const handleCenterSelected = useCallback(() => {
    if (!selectedBook) return;
    const center = centerToMapPoint(selectedItem, centroidForBook(selectedBook));
    zoomRef.current = 2.2;
  ...
  const handlePointerDown = useCallback((event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    const artCenter = centerToMapPoint(selectedItem, centroidForBook(selectedBook));
  ...
  return (
    <BuilderWorkspace
      route="curate"
      title="Curate"
      subtitle="Place, size, and rotate constellation artwork over the assigned sky before refining stars."
      sidebarWidthClass="w-80"
      sidebar={
        <>
          <p className="text-xs leading-relaxed text-white/45">
            Curate consumes the assigned `arrangement.json` draft and authors the artwork fields in `constellations.json`.
          </p>
